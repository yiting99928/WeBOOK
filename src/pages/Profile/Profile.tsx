...
const Profile = () => {
  ...
  const [groupData, setGroupData] = useState<StudyGroupType[]>([]);
  ...
  async function getData() {
    if (!user) return;
    const groupData = await data.getUserGroup(user.email as string);
    let filteredData;
    if (!status) {
  ...
  async function handleQuitGroup(id: string) {
  ...
  function handleExpanded(index: number) {
...
const LoadingContent = styled.div`
  ...
  width: 600px;
`;
const Info = styled(Pulse)<{ $height: string; $width: string }>`
...
`;
const GroupInfo = styled.div<{ $expanded: boolean }>`
  display: flex;
  gap: 50px;
