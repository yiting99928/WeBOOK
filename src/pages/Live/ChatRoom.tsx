...
const Message = () => {
  ...
  return (
    <MessageStyled>
      {messages.map(({ sender, message, senderImg, senderName }, index) => {
        if (sender === null)
        ...
        return (
          <GuestMessage key={index}>
            <UserImg src={senderImg as string} alt="userImg" />
            <SenderMessage>
...
const Chat = () => {
  ...
  const sendMessage = useCallback(
    async (e: FormEvent) => {
      ...
      if (trimmedMessage) {
        await data.addMessage(id as string, {
...
export const ChatRoom = React.memo(
  ({ showChatRoom }: { showChatRoom: boolean }) => (
...
const SenderMessage = styled.div`
  ...
  border-radius: 6px;
`;
const ChatRoomContainer = styled.div<{ $showChatRoom: boolean }>`
