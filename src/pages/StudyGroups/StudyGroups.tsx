...
function StudyGroups() {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      const groups = (await data.getUnfinishedGroups()) as StudyGroupType[];
  ...
  const handleSearchByCategory = async (category: CategoryType) => {
    let groups;
    if (category === '全部讀書會') {
    ...
    groups = groups as StudyGroupType[];

    setSelectedCategory(category);
    setAllGroupsData(groups);
  };

  const searchByText = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  function filterGroups(groups: StudyGroupType[], filterOption: string) {
