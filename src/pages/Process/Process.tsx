...
function Process() {
  ...
  useEffect(() => {
    async function initData() {
      try {
        if (!id) return;
        const studyGroupData = (await data.getGroup(id)) as StudyGroupType;
        setStudyGroup(studyGroupData);

        const templatesData = (await data.getTemplates()) as TemplateType[];
  ...
  const handleDragStart = (
    ...
    e.dataTransfer.setData('text/plain', processIndex.toString());
  };
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>, processIndex: number) => {
    e.preventDefault();
  ...
  function areAllQAsValid(processData: TemplateType[]) {
  ...
  function handelSave(processData: TemplateType[]) {
  ...
  return (
    <div>
      <Container>
        <TitleContainer>
          {studyGroup && <GroupTitle studyGroup={studyGroup} />}
        </TitleContainer>
        <ProcessContainer>
...
`;
const Buttons = styled.div<{ $editable: boolean }>`
...
const ProcessCard = styled.div`
  ...
  padding: 30px;
`;
const EditBlock = styled.div<{ $editable: boolean }>`
  position: absolute;
