...
const MobileBg = styled.div<{ $isOpen: boolean }>`
  height: 100vh;
  width: 100vw;
  position: fixed;
  top: 0;
...
const HamburgerLine1 = styled.div<{ $isOpen: boolean }>`
  ${lineStyles}
...
const HamburgerLine2 = styled.div<{ $isOpen: boolean }>`
  ${lineStyles}
  display: ${({ $isOpen }) => ($isOpen ? 'none' : 'block')};
`;

const HamburgerLine3 = styled.div<{ $isOpen: boolean }>`
  ${lineStyles}
  transform: ${({ $isOpen }) => ($isOpen ? 'rotate(-45deg)' : 'rotate(0)')};
  position: ${({ $isOpen }) => ($isOpen ? 'absolute' : 'static')};
  top: ${({ $isOpen }) => ($isOpen ? 'calc(50% - 1.5px)' : 'auto')};
...
const MobileWrap = styled.div<{ $isOpen: boolean }>`
  display: ${({ $isOpen }) => ($isOpen ? 'block' : 'none')};
  font-size: 18px;
  position: fixed;
